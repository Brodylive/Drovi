...
<head>
	...
	<title>Modifier un commerce | Drovi</title>
...
<div id="overlay" style="display:none;" onclick="$('#overlay, #popupfortop').hide();"></div>
	...
	<header>
		...
		?>
			...
			<a href="compte.php" sid="nomuser" style="text-decoration:none;margin-top:8px;margin-right:10px;float:right;">Salut <? echo $nomuser; ?>!</a>
		<?
		} else {
		?>
	...
	<div id="content">
		...
		<h2 class="ajouteruncommerce">Modifier un commerce</h2>
...
<?	
	if ($_POST['action']=='modifier') {
		...
		$reponse = $bdd->query("UPDATE `drovi_magasins` SET `nom`='$nom', `description`='$description', `adresse`='$adresse', `latLng`='$latlng', `cat`='$cat', `1`=$lundi, `2`=$mardi, `3`=$mercredi, `4`=$jeudi, `5`=$vendredi, `6`=$samedi, `0`=$dimanche WHERE `id`=$id");
		...
		$to = $row['email'];
		$subject = 'Tu as modifié un commerce sur Drovi';
...
<body style='margin:0;padding:0; font-family:arial, sans-serif;' bgcolor='#fff' color='#fff'>
					...
					Salut &agrave; toi $username!</h1>
						...
						<tr>
							<td style='color:#fff; border-bottom-style: solid; border-bottom-color: #3498db; border-bottom-width: 3px;' width='500'>
									...
									Tu viens de modifier un commerce sur <a href='http://jenniferdenis.be/tfe/prototype/' style='text-decoration: none; color:#ddd;'> Drovi</a><br>
	...
	<form method="POST">
		...
		<label>Lundi</label>
		<input type="text" value="<? echo $lundi; ?>" readonly="readonly">
		
		<label>Mardi</label>
		<input type="text" value="<? echo $mardi; ?>" readonly="readonly">
		
		<label>Mercredi</label>
		<input type="text" value="<? echo $mercredi; ?>" readonly="readonly">
		
		<label>Jeudi</label>
		<input type="text" value="<? echo $jeudi; ?>" readonly="readonly">
		
		<label>Vendredi</label>
		<input type="text" value="<? echo $vendredi; ?>" readonly="readonly">
		
		<label>Samedi</label>
		<input type="text" value="<? echo $samedi; ?>" readonly="readonly">
		
		<label>Dimanche</label>
		<input type="text" value="<? echo $dimanche; ?>" readonly="readonly">
	...
	<?
		...
		$searchmag= $bdd->query("SELECT * FROM drovi_magasins WHERE id='$id'");
		$row=$searchmag->fetch();
		...
		if($lundi){
			if(preg_match('/and/', $lundi)){
				
				$timesopen=explode(' and ', $lundi);		
				
				$timeopenlundi=explode(' to ', $timesopen[0]);
				$timeopenlundi2=explode(' to ', $timesopen[1]);
				
				$timea=substr($timeopenlundi[0], 0, -3)*60+substr($timeopenlundi[0], -5, 2);
				$timeb=substr($timeopenlundi[1], 0, -3)*60+substr($timeopenlundi[1], -5, 2);	
				
				$timelundi=$timea.";".$timeb;
				
				$time2a=substr($timeopenlundi2[0], 0, -3)*60+substr($timeopenlundi2[0], -5, 2);
				$time2b=substr($timeopenlundi2[1], 0, -3)*60+substr($timeopenlundi2[1], -5, 2);	
				
				$timelundi2=$time2a.";".$time2b;
				
			} else {
				$timeopenlundi=explode(' to ', $lundi);
				
				$timea=substr($timeopenlundi[0], 0, -3)*60+substr($timeopenlundi2[0], -5, 2);
				$timeb=substr($timeopenlundi[1], 0, -3)*60+substr($timeopenlundi2[1], -5, 2);	
		...
		if($mardi){
			if(preg_match('/and/', $mardi)){
				
				$timesopen=explode(' and ', $mardi);		
				
				$timeopenmardi=explode(' to ', $timesopen[0]);
				$timeopenmardi2=explode(' to ', $timesopen[1]);
				
				$timea=substr($timeopenmardi[0], 0, -3)*60+substr($timeopenmardi[0], -5, 2);
				$timeb=substr($timeopenmardi[1], 0, -3)*60+substr($timeopenmardi[1], -5, 2);	
				
				$timemardi=$timea.";".$timeb;
				
				$time2a=substr($timeopenmardi2[0], 0, -3)*60+substr($timeopenmardi2[0], -5, 2);
				$time2b=substr($timeopenmardi2[1], 0, -3)*60+substr($timeopenmardi2[1], -5, 2);	
				
				$timemardi2=$time2a.";".$time2b;

				
			} else {
				$timeopenmardi=explode(' to ', $mardi);
		...
		if($mercredi){
			if(preg_match('/and/', $mercredi)){
				
				$timesopen=explode(' and ', $mercredi);		
				
				$timeopenmercredi=explode(' to ', $timesopen[0]);
				$timeopenmercredi2=explode(' to ', $timesopen[1]);
				
				$timea=substr($timeopenmercredi[0], 0, -3)*60+substr($timeopenmercredi[0], -5, 2);
				$timeb=substr($timeopenmercredi[1], 0, -3)*60+substr($timeopenmercredi[1], -5, 2);	
				
				$timemercredi=$timea.";".$timeb;
				
				$time2a=substr($timeopenmercredi2[0], 0, -3)*60+substr($timeopenmercredi2[0], -5, 2);
				$time2b=substr($timeopenmercredi2[1], 0, -3)*60+substr($timeopenmercredi2[1], -5, 2);	
				
				$timemercredi2=$time2a.";".$time2b;

				
			} else {
				$timeopenmercredi=explode(' to ', $mercredi);
		...
		if($jeudi){
			if(preg_match('/and/', $jeudi)){
				
				$timesopen=explode(' and ', $jeudi);		
				
				$timeopenjeudi=explode(' to ', $timesopen[0]);
				$timeopenjeudi2=explode(' to ', $timesopen[1]);
				
				$timea=substr($timeopenjeudi[0], 0, -3)*60+substr($timeopenjeudi[0], -5, 2);
				$timeb=substr($timeopenjeudi[1], 0, -3)*60+substr($timeopenjeudi[1], -5, 2);	
				
				$timejeudi=$timea.";".$timeb;
				
				$time2a=substr($timeopenjeudi2[0], 0, -3)*60+substr($timeopenjeudi2[0], -5, 2);
				$time2b=substr($timeopenjeudi2[1], 0, -3)*60+substr($timeopenjeudi2[1], -5, 2);	
				
				$timejeudi2=$time2a.";".$time2b;
				
			} else {
				$timeopenjeudi=explode(' to ', $jeudi);
		...
		if($vendredi){
			if(preg_match('/and/', $vendredi)){
				
				$timesopen=explode(' and ', $vendredi);		
				
				$timeopenvendredi=explode(' to ', $timesopen[0]);
				$timeopenvendredi2=explode(' to ', $timesopen[1]);
				
				$timea=substr($timeopenvendredi[0], 0, -3)*60+substr($timeopenvendredi[0], -5, 2);
				$timeb=substr($timeopenvendredi[1], 0, -3)*60+substr($timeopenvendredi[1], -5, 2);	
				
				$timevendredi=$timea.";".$timeb;
				
				$time2a=substr($timeopenvendredi2[0], 0, -3)*60+substr($timeopenvendredi2[0], -5, 2);
				$time2b=substr($timeopenvendredi2[1], 0, -3)*60+substr($timeopenvendredi2[1], -5, 2);	
				
				$timevendredi2=$time2a.";".$time2b;
				
			} else {
				$timeopenvendredi=explode(' to ', $vendredi);
		...
		if($samedi){
			if(preg_match('/and/', $samedi)){
				
				$timesopen=explode(' and ', $samedi);		
				
				$timeopensamedi=explode(' to ', $timesopen[0]);
				$timeopensamedi2=explode(' to ', $timesopen[1]);
				
				$timea=substr($timeopensamedi[0], 0, -3)*60+substr($timeopensamedi[0], -5, 2);
				$timeb=substr($timeopensamedi[1], 0, -3)*60+substr($timeopensamedi[1], -5, 2);	
				
				$timesamedi=$timea.";".$timeb;
				
				$time2a=substr($timeopensamedi2[0], 0, -3)*60+substr($timeopensamedi2[0], -5, 2);
				$time2b=substr($timeopensamedi2[1], 0, -3)*60+substr($timeopensamedi2[1], -5, 2);	
				
				$timesamedi2=$time2a.";".$time2b;
				
			} else {
				$timeopensamedi=explode(' to ', $samedi);
		...
		if($dimanche){
			if(preg_match('/and/', $dimanche)){
				
				$timesopen=explode(' and ', $dimanche);		
				
				$timeopendimanche=explode(' to ', $timesopen[0]);
				$timeopendimanche2=explode(' to ', $timesopen[1]);
				
				$timea=substr($timeopendimanche[0], 0, -3)*60+substr($timeopendimanche[0], -5, 2);
				$timeb=substr($timeopendimanche[1], 0, -3)*60+substr($timeopendimanche[1], -5, 2);	
				
				$timedimanche=$timea.";".$timeb;
				
				$time2a=substr($timeopendimanche2[0], 0, -3)*60+substr($timeopendimanche2[0], -5, 2);
				$time2b=substr($timeopendimanche2[1], 0, -3)*60+substr($timeopendimanche2[1], -5, 2);	
				
				$timedimanche2=$time2a.";".$time2b;
				
			} else {
				$timeopendimanche=explode(' to ', $dimanche);
...
?>
		...
		<input type="text" name="description" value="<? echo $description; ?>">
		
		<label for="adresse">Adresse du magasin</label>
		<input type="text" id="adresse" name="adresse" onchange="getAdresse();" value="<? echo $adresse; ?>">
		...
		<input type="text" id="latlng" name="latlng" value="<? echo $latlng; ?>">
		...
		<input type="checkbox" id="trigger1" name="lundi" <? if($lundi) echo "checked"; ?> class="checkbox-11-2">
		<label for="trigger1"></label>
		
		<input type="hidden" id="lundi_debut" name="lundi_debut">
		<input type="hidden" id="lundi_fin" name="lundi_fin">

	    <div class="layout-slider" id="layout-slider1" <? if($lundi) echo "style='display:block;'"; ?>>
	      <input id="Slider1" type="slider" name="area" value="<? if($lundi) echo $timelundi; else echo "480;1080" ?>" />
	      <input type="checkbox" id="openSlider12" <? if($timelundi2) echo "checked"; ?> name="lundi2"><label for="openSlider12"><? if($timelundi2) echo "Supprimer cette tranche horaire"; else echo "Ajouter une nouvelle tranche horaire"; ?></label>
	    </div>
	    
	    <input type="hidden" id="lundi_debut2" name="lundi_debut2">
		<input type="hidden" id="lundi_fin2" name="lundi_fin2">
	    
	    <div class="layout-slider" id="layout-slider12" <? if($timelundi2) echo "style='display:block;'"; ?>>
	      <input id="Slider12" type="slider" name="area" value="<? if($timelundi2) echo $timelundi2; else echo "480;1080" ?>" />
	    </div>
	    
	    <p class="day">Mardi</p>
		
		<input type="checkbox" id="trigger2" name="mardi" <? if($mardi) echo "checked"; ?> class="checkbox-11-2">
		<label for="trigger2"></label>
		
		<input type="hidden" id="mardi_debut" name="mardi_debut">
		<input type="hidden" id="mardi_fin" name="mardi_fin">

	    <div class="layout-slider" id="layout-slider2" <? if($mardi) echo "style='display:block;'"; ?>>
	      <input id="Slider2" type="slider" name="area" value="<? if($mardi) echo $timemardi; else echo "480;1080" ?>" />
	      <input type="checkbox" id="openSlider22" <? if($timemardi2) echo "checked"; ?> name="mardi2"><label for="openSlider22"><? if($timemardi2) echo "Supprimer cette tranche horaire"; else echo "Ajouter une nouvelle tranche horaire"; ?></label>
	    </div>
	    
	    <input type="hidden" id="mardi_debut2" name="mardi_debut2">
		<input type="hidden" id="mardi_fin2" name="mardi_fin2">
	    
	    <div class="layout-slider" id="layout-slider22" <? if($timemardi2) echo "style='display:block;'"; ?>>
	      <input id="Slider22" type="slider" name="area" value="<? if($timemardi2) echo $timemardi2; else echo "480;1080" ?>" />
	    </div>
		...
		<input type="checkbox" id="trigger3" name="mercredi" <? if($mercredi) echo "checked"; ?> class="checkbox-11-2">
	    ...
	    <div class="layout-slider" id="layout-slider3" <? if($mercredi) echo "style='display:block;'"; ?>>
	      <input id="Slider3" type="slider" name="area" value="<? if($mercredi) echo $timemercredi; else echo "480;1080" ?>" />
	      <input type="checkbox" id="openSlider32" <? if($timemercredi2) echo "checked"; ?> name="mercredi2"><label for="openSlider32"><? if($timemercredi2) echo "Supprimer cette tranche horaire"; else echo "Ajouter une nouvelle tranche horaire"; ?></label>
	    </div>
	    
	    <input type="hidden" id="mercredi_debut2" name="mercredi_debut2">
		<input type="hidden" id="mercredi_fin2" name="mercredi_fin2">
	    
	    <div class="layout-slider" id="layout-slider32" <? if($timemercredi2) echo "style='display:block;'"; ?>>
	      <input id="Slider32" type="slider" name="area" value="<? if($timemercredi2) echo $timemercredi2; else echo "480;1080" ?>" />
	    </div>
		...
		<input type="checkbox" id="trigger4" name="jeudi" <? if($jeudi) echo "checked"; ?> class="checkbox-11-2">
		<label for="trigger4"></label>
		
		<input type="hidden" id="jeudi_debut" name="jeudi_debut">
		<input type="hidden" id="jeudi_fin" name="jeudi_fin">

	    <div class="layout-slider" id="layout-slider4" <? if($jeudi) echo "style='display:block;'"; ?>>
	      <input id="Slider4" type="slider" name="area" value="<? if($jeudi) echo $timejeudi; else echo "480;1080" ?>" />
	      <input type="checkbox" id="openSlider42" <? if($timejeudi2) echo "checked"; ?> name="jeudi2"><label for="openSlider42"><? if($timejeudi2) echo "Supprimer cette tranche horaire"; else echo "Ajouter une nouvelle tranche horaire"; ?></label>
	    </div>
	    
	    <input type="hidden" id="jeudi_debut2" name="jeudi_debut2">
		<input type="hidden" id="jeudi_fin2" name="jeudi_fin2">
	    
	    <div class="layout-slider" id="layout-slider42" <? if($timejeudi2) echo "style='display:block;'"; ?>>
	      <input id="Slider42" type="slider" name="area" value="<? if($timejeudi2) echo $timejeudi2; else echo "480;1080" ?>" />
	    </div>
		...
		<input type="checkbox" id="trigger5" name="vendredi" <? if($vendredi) echo "checked"; ?> class="checkbox-11-2">
	    ...
	    <div class="layout-slider" id="layout-slider5" <? if($vendredi) echo "style='display:block;'"; ?>>
	      <input id="Slider5" type="slider" name="area" value="<? if($vendredi) echo $timevendredi; else echo "480;1080" ?>" />
	      <input type="checkbox" id="openSlider52" <? if($timevendredi2) echo "checked"; ?> name="vendredi2"><label for="openSlider52"><? if($timevendredi2) echo "Supprimer cette tranche horaire"; else echo "Ajouter une nouvelle tranche horaire"; ?></label>
	    </div>
	    
	    <input type="hidden" id="vendredi_debut2" name="vendredi_debut2">
		<input type="hidden" id="vendredi_fin2" name="vendredi_fin2">
	    
	    <div class="layout-slider" id="layout-slider52" <? if($timevendredi2) echo "style='display:block;'"; ?>>
	      <input id="Slider52" type="slider" name="area" value="<? if($timevendredi2) echo $timevendredi2; else echo "480;1080" ?>" />
	    </div>
	    
	    
	    <p class="day">Samedi</p>
		
		<input type="checkbox" id="trigger6" name="samedi" <? if($samedi) echo "checked"; ?> class="checkbox-11-2">
	    ...
	    <div class="layout-slider" id="layout-slider6" <? if($samedi) echo "style='display:block;'"; ?>>
	      <input id="Slider6" type="slider" name="area" value="<? if($samedi) echo $timesamedi; else echo "480;1080" ?>" />
	      <input type="checkbox" id="openSlider62" <? if($timesamedi2) echo "checked"; ?> name="samedi2"><label for="openSlider62"><? if($timesamedi2) echo "Supprimer cette tranche horaire"; else echo "Ajouter une nouvelle tranche horaire"; ?></label>
	    </div>
	    
	    <input type="hidden" id="samedi_debut2" name="samedi_debut2">
		<input type="hidden" id="samedi_fin2" name="samedi_fin2">
	    
	    <div class="layout-slider" id="layout-slider62" <? if($timesamedi2) echo "style='display:block;'"; ?>>
	      <input id="Slider62" type="slider" name="area" value="<? if($timesamedi2) echo $timesamedi2; else echo "480;1080" ?>" />
	    </div>
		...
		<input type="checkbox" id="trigger0" name="dimanche" <? if($dimanche) echo "checked"; ?> class="checkbox-11-2">
	    ...
	    <div class="layout-slider" id="layout-slider0" <? if($dimanche) echo "style='display:block;'"; ?>>
	      <input id="Slider0" type="slider" name="area" value="<? if($dimanche) echo $timedimanche; else echo "480;1080" ?>" />
	      <input type="checkbox" id="openSlider02" <? if($timedimanche2) echo "checked"; ?> name="dimanche2"><label for="openSlider02"><? if($timedimanche2) echo "Supprimer cette tranche horaire"; else echo "Ajouter une nouvelle tranche horaire"; ?></label>
	    </div>
	    
	    <input type="hidden" id="dimanche_debut2" name="dimanche_debut2">
		<input type="hidden" id="dimanche_fin2" name="dimanche_fin2">
	    
	    <div class="layout-slider" id="layout-slider02" <? if($timedimanche2) echo "style='display:block;'"; ?>>
	      <input id="Slider02" type="slider" name="area" value="<? if($timedimanche2) echo $timedimanche2; else echo "480;1080" ?>" />
	    </div>	    
	...
	$(document).ready(function(){
      ...
      $("#triggerj10").click( function(){
		...
		<input type="submit" value="Modifier ce commerce">
	</form>
		...
		function GeocodingResult( results , status )
		{
		  ...
		  if( status == google.maps.GeocoderStatus.OK ) {
		    ...
		    flag_latlng=true;
			  ...
			  document.getElementById("adresse").style.background="#2ecc71";
